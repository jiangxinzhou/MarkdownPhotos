...
public:
	...
	void setOptions(const vector<string> &vecOption) {
		...
		for (; i < vecOption.size(); ++i) {
			...
			if (pr.first == "lstm_output_size")
				lstm_output_size = atoi(pr.second.c_str());
			if (pr.first == "mlp_size")
				mlp_size = atoi(pr.second.c_str());
			if (pr.first == "mlp_rel_size")
				mlp_rel_size = atoi(pr.second.c_str());
			...
			if (pr.first == "momentum")
				momentum = atof(pr.second.c_str());
			if (pr.first == "clip")
			...
			if (pr.first == "wordEmbSize")
				wordEmbeddingDim = atoi(pr.second.c_str());
			if (pr.first == "posEmbSize")
				posEmbeddingDim = atoi(pr.second.c_str());
	...
	void showOptions() {
		...
		std::cout << "wordEmbeddingDim = " << wordEmbeddingDim << std::endl;
		std::cout << "posEmbeddingDim = " << posEmbeddingDim << std::endl;
